...
public class JFXArena extends Pane
{
    ...
    private List<Robot> robots_list = new ArrayList<>();

    private ExecutorService thread_pool = Executors.newFixedThreadPool(100);

    private BlockingQueue<Runnable>  acquired_tiles = new ArrayBlockingQueue<>(gridWidth*gridHeight);

    private Object mutex_corner = new Object();
    private boolean corner1 = true;
    private boolean corner2 = true;
    private boolean corner3 = true;
    private boolean corner4 = true;
...
//            System.out.println("robots_list.size() = "+ robots_list.size() );
            ...
            for(Robot r : robots_list){
               ...
               // System.out.println(r.toString());
                ...
                drawImage(gfx, robot1, r.getCoordX(), r.getCoordY());
                drawLabel(gfx, r.getName(), r.getCoordX(), r.getCoordY());
            }
        }
    ...
    private void generate_Robots(double coordX, double coordY){

        try {
                        Robot rbDummy = new Robot(robotID, coordX, coordY);
                        robots_list.add(rbDummy);
                        Thread robot = new Thread(rbDummy);
                        thread_pool.execute(robot);
                        requestLayout();
...
//    private double[] getAvailableTiles(){
//        double available_coords[];
//
//        int corners[] = {0, gridWidth, (gridWidth*gridHeight - gridWidth), gridWidth*gridHeight};
//
//        for(Robot r : robots_list){
//            if((r.getCoordX() != 0 && r.getCoordY() != 0)){
//                available_coords
//            }
//        }
//
